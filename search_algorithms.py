...
def load_sorted_data(size: int) -> list[int]:
    sorted_path = os.path.join(DATA_DIR, f"data_sorted_{size}.txt")
    raw_path    = os.path.join(DATA_DIR, f"data_{size}.txt")

    if os.path.exists(sorted_path):
        path = sorted_path
    elif os.path.exists(raw_path):
        print(f"Ordenando datos para tamaño {size}...")
        with open(raw_path, encoding="utf-8") as f:
            data = sorted(int(line.strip()) for line in f if line.strip())
        with open(sorted_path, "w", encoding="utf-8") as f:
            f.write("\n".join(map(str, data)))
        return data
    else:
        print(f"Archivo no encontrado: {sorted_path}")
        return []
